...
function SearchBar({
    ...
    useEffect(() => {
        if (value && value.trim() !== '' && value !== history[currentIndex]) {
            ...
            if (currentIndex < history.length - 1) newHistory.splice(currentIndex + 1);
    ...
    const handleAutoComboboxBlur = (blurredValue) => {
        if (blurredValue?.trim()) handleNavigation(blurredValue.trim());
    };
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (searchValue.trim()) handleNavigation(searchValue.trim());
    };
    ...
    const handleBack = () => {
        if (currentIndex > 0) {
            ...
            try { sessionStorage.setItem('cowSearchCurrentIndex', newIndex.toString()); }
            catch (error) { console.error('Error saving search index:', error); }
            setCurrentIndex(newIndex);
    ...
    const buttonStyle = {
        ...
        border: 'none', borderRadius: '3px', cursor: 'pointer', minWidth: '80px',
    };
